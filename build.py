...
# Environment detection: --prod flag or GITHUB_ACTIONS env
is_prod = "--prod" in sys.argv or os.environ.get("GITHUB_ACTIONS") == "true"
...
def main():
    mode_str = "PRODUCTION" if is_prod else "DEVELOPMENT"
    print(f"[Build] Starting Solarized Light Blog Compiler [{mode_str} mode]...")
    ...
    for filename in post_files:
        ...
        # Check dev status and env
        is_dev_post = str(metadata.get("dev", False)).lower() == "true"
        if is_dev_post and is_prod:
            print(f"[Build] Skipping dev draft: {filename}")
            continue
            
        ai_usage = str(metadata.get("ai_usage", "no-ai")).lower().strip()
        if ai_usage not in ["no-ai", "mixed", "vibecoded"]:
            print(f"[WARN] Unknown ai_usage '{ai_usage}' in {filename}. Defaulting to 'no-ai'.")
            ai_usage = "no-ai"
            
        # Enforce validation for mixed AI usage via front-matter details
        ai_usage_details = metadata.get("ai_usage_details") or metadata.get("ai-usage-details", "")
        if ai_usage == "mixed":
            if not ai_usage_details or not str(ai_usage_details).strip():
                print(f"[ERR] Validation failed: Post '{filename}' has ai_usage='mixed' but is missing the required 'ai_usage_details' front matter field.")
                sys.exit(1)
        ...
        # Add AI tag to tags for classification filtering
        if ai_usage == "no-ai":
            tags.append("no-ai")
        elif ai_usage == "vibecoded":
            tags.append("vibecoded")
        elif ai_usage == "mixed":
            tags.append("mixed-ai")
        ...
        # Inject AI disclosures callouts
        if ai_usage == "no-ai":
            text = str(ai_usage_details).strip() if ai_usage_details else "This development article was written entirely by hand without any AI assistance."
            content_html = markdown.markdown(text, extensions=["fenced_code", "tables"])
            callout_html = f"""<div class="ai-disclosure no-ai-disclosure">
          <svg viewBox="0 0 24 24"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>
          <div>
            <strong>Human Authored</strong><br>
            {content_html}
          </div>
        </div>"""
            html_content = callout_html + "\n" + html_content
        elif ai_usage == "vibecoded":
            text = str(ai_usage_details).strip() if ai_usage_details else "This article was fully generated by an AI assistant during a development session."
            content_html = markdown.markdown(text, extensions=["fenced_code", "tables"])
            callout_html = f"""<div class="ai-disclosure vibecoded-disclosure">
          <svg viewBox="0 0 24 24"><path d="M19 8h-2V7c0-2.76-2.24-5-5-5S7 4.24 7 7v1H5c-1.1 0-2 .9-2 2v8c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2v-8c0-1.1-.9-2-2-2zm-7-4c1.66 0 3 1.34 3 3v1H9V7c0-1.66 1.34-3 3-3zm7 14H5v-8h14v8zm-9-5c-.55 0-1 .45-1 1s.45 1 1 1 1-.45 1-1-.45-1-1-1zm4 0c-.55 0-1 .45-1 1s.45 1 1 1 1-.45 1-1-.45-1-1-1z"/></svg>
          <div>
            <strong>Fully Vibecoded</strong><br>
            {content_html}
          </div>
        </div>"""
            html_content = callout_html + "\n" + html_content
        elif ai_usage == "mixed":
            # Compile the mixed content markdown to HTML and wrap it in mixed callout container
            ai_content_html = markdown.markdown(
                str(ai_usage_details).strip(), 
                extensions=["fenced_code", "tables"]
            )
            callout_html = f"""<div class="ai-disclosure mixed-disclosure">
          ...
          <div>
            <strong>Mixed AI Usage</strong><br>
            {ai_content_html}
          </div>
        </div>"""
            html_content = callout_html + "\n" + html_content
